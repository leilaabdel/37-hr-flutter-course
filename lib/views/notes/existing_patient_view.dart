...
class ExistingPatientView extends StatefulWidget {
  final PatientRecord patient;
  const ExistingPatientView({super.key, required this.patient});

  @override
  ExistingPatientViewState createState() {
    // ignore: no_logic_in_create_state
    return ExistingPatientViewState(patient: patient);
  }
}

// Create a corresponding State class.
// This class holds data related to the form.
class ExistingPatientViewState extends State<ExistingPatientView> {
  // Create a global key that uniquely identifies the Form widget
  // and allows validation of the form.
  //
  // Note: This is a GlobalKey<FormState>,
  // not a GlobalKey<ExistingPatientViewState>.
  ...
  void initState() {
    ...
    firstName.text = patient.first_name;
    sex.text = patient.sex ?? "";
    address1.text = patient.address_1 ?? "";
    address2.text = patient.address_2 ?? "";
    occupation.text = patient.occupation ?? "";
    maritalStatus.text = patient.marital_status ?? "";
    dob.text =
        patient.dob == null ? "" : DateFormat('MM-dd-yy').format(patient.dob!);
    age.text = patient.age.toString();
    placeOfBirth.text = patient.place_of_birth ?? "";
    // TODO: DO TRIBE IMPLEMENTATION
    patientContact.text = patient.contact ?? "";
    religion.text = patient.religion ?? "";
    nextOfKin.text = patient.next_of_kin ?? "";
    addressOfNextofKin.text = patient.address_of_next_of_kin ?? "";
    dobInput.text =
        patient.dob == null ? "" : DateFormat('MM-dd-yy').format(patient.dob!);
    dateOfFirstVisitInput.text = patient.date_of_first_attendance == null
        ? ""
        : DateFormat('MM-dd-yy').format(patient.date_of_first_attendance!);
    dobDate = patient.dob ?? DateTime.now();
    dateOfFirstVisit = patient.date_of_first_attendance ?? DateTime.now();
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        if (_formKey.currentState!.validate()) {
                          ...
                          var patientCloud = PatientCloudRecord(
                            id: patient.id,
                          ...
                          PatientService().updatePatient(patient: patientCloud);
                          PatientService()
                              .writeToFireStore(patientDoc: patientCloud);
